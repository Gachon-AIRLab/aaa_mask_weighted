...
def maskrcnn_loss(mask_logits, proposals, gt_masks, gt_labels, mask_matched_idxs):
    ...
    #  n x 2 x 28 x 28 이므로, mask_logits.shape[-1] = 28
    discretization_size = mask_logits.shape[-1]

    # label  이의미하 는바 는무엇? 왜 2개 의list 로 되어있나?
    labels = [l[idxs] for l, idxs in zip(gt_labels, mask_matched_idxs)]

    # who define "box"? proposal?
    mask_targets = [
    ...
    mask_logits_softmax = F.softmax(mask_logits, dim=1)
    # print(mask_logits.shape)
    # print(mask_logits_softmax.shape)

    mask_logit_softmax_proposal = mask_logits_softmax[:,1,:,:]
    # print(mask_logit_softmax_proposal.shape)
    # print(mask_targets.shape)
    #
    # print(torch.max(mask_targets))
    # print(torch.min(mask_targets))
    # mask_loss = F.smooth_l1_loss(mask_logits[:,1,:,:], mask_targets) * 0.01
    # mask_loss = F.smooth_l1_loss(mask_logit_softmax_proposal, mask_targets) * 10.0
    mask_loss = F.mse_loss(mask_logit_softmax_proposal, mask_targets) * 10.0
